...
async def save_roadmap(
    ...
    try:
        ...
        if existing_roadmap:
            ...
            roadmap = await roadmaps_collection.find_one({"_id": roadmap_insert.inserted_id})
        ...
        if existing_user_roadmap:
            ...
            user_roadmap_insert = await user_roadmaps_collection.insert_one(user_roadmap_data)
            user_roadmap = await user_roadmaps_collection.find_one(
...
async def parse_and_save_roadmap(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to parse and save: {str(e)}")
...
async def get_user_roadmaps(
    ...
    try:
        ...
        # Fetch user roadmaps
        cursor = user_roadmaps_collection.find({"userId": user_id}).sort("lastAccessed", -1)
        ...
        if week_number is not None:
            ...
            for week in weekly_progress:
                if week["weekNumber"] == week_number:
                    week["isCompleted"] = is_completed if is_completed is not None else week["isCompleted"]
                    if is_completed:
            ...
            completed_weeks = sum(1 for w in weekly_progress if w.get("isCompleted", False))
            update_data["overallProgress"] = round((completed_weeks / total_weeks) * 100) if total_weeks > 0 else 0
